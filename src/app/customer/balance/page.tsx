  ...
  return (
    <div className="space-y-8">
      <CabinetPageHeader
        ...
        description="Пополнение баланса и вывод средств. Безопасная сделка: сумма резервируется под конкретный заказ до приёмки работы, затем учитывается в пользу исполнителя (за вычетом комиссии). Ниже — история операций."
      />
          ...
          {txs.length === 0 ? (
            <div className="p-4">
              <CabinetEmptyState
                ...
                description="Здесь появятся пополнения, резерв суммы под заказы (безопасная сделка) и списания. Пополнить баланс можно в блоке выше; после работы с заказами строки заполнятся автоматически."
              >
